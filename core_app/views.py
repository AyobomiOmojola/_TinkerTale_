...
class HomeView(APIView):
    ...
    @swagger_auto_schema(operation_summary="This serves data for the home page", operation_description="THIS ENDPOINT SERVES DATA DISPLAYED IN THE HOMEPAGE. FOR NON-AUTHENTICATED USERS AND AUTHENTICATED USERS BELOW THE AGE OF 18 AS DETERMINED FROM THEIR USERPROFILE, THE STORIES DISPLAYED TO SUCH PERSONS ARE ONES RATED 'G' (GENERAL AUDIENCE); THESE ARE STORIES WITH NON-EXPLICIT CONTENTS(FOR NON AUTHENTICATED USERS, THIS IS SIMPLY TO PLAY IT SAFE AS SUCH PERSONS AGE GROUP CANNOT BE DETERMINED). FOR AUTHENTICATED USERS ABOVE THE AGE OF 18, THE STORIES DISPLAYED TO SUCH ARE RATED BOTH 'G' AND 'R'(RESTRICTED), IN CONTRAST TO 'G' RATED STORIES, 'R' RATED STORIES ARE ONES WITH EXPLICIT CONTENTS AND NOT FIT FOR GENERAL VIEW. OTHER CATEGORIES OF STORIES ARE ALSO DISPLAYED SUCH AS TOP 10 LATEST AND FAVORITE STORIES(ALL ON THE BASIS OF THE GENRE THE REQUEST_USER IS INTERESTED IN) AND STORIES WRITTEN BY THE USERS THE REQUEST_USER IS FOLLOWING.\n\n NOTE: STORIES WRITTEN BY THE REQUEST_USER IS NOT DISPLAYED ON THEIR HOMEPAGE",tags=['Home Page'] )
